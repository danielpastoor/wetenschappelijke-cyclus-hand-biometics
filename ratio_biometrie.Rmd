...
biometric_dats <- cbind(biometric_dat, sekse)
...
# determine ratio 
breedte_man_mean <- mean((data[data$sexse == "man", ])$breedte)
breedte_vrouw_mean <- mean((data[data$sexse == "vrouw", ])$breedte)
lengte_man_mean <- mean((data[data$sexse == "man", ])$lengte)
lengte_vrouw_mean <- mean((data[data$sexse == "vrouw", ])$lengte)
ratio_vrouw <- (lengte_vrouw_mean/breedte_vrouw_mean)
ratio_man <- (lengte_man_mean/breedte_man_mean)
```
### Dit moet ff ingevuld worden:)
...
Ze zijn niet significant verschillende, maar er is wel een verschil aanwezig. Dit komt doordat de persoon die gemeten heeft op een andere manier kan kijken vanaf boven en zo een andere lengte ziet. En het ligt er aan hoe accuraat de persoon kijkt.
